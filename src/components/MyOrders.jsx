...
const MyOrders = ({ user, orders, products, logout, isAdmin, collections, reservations, updateOrderPaymentProof, showToast, notifications, markNotificationRead, markAllNotificationsRead, dataLoading }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
                ...
                <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-2 text-[12px] text-gray-600 hover:text-gray-900 mb-4 sm:mb-6 transition font-medium"
                >
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-4 sm:mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <h1 className="text-base sm:text-lg font-bold text-gray-900">My Orders</h1>
                        ...
                        <select
                            ...
                            className="px-3 sm:px-4 py-1.5 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent-600 focus:outline-none text-xs sm:text-sm bg-white"
                        >
                ...
                {dataLoading?.orders ? (
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-2 btn-accent px-4 py-2 text-sm font-semibold"
                        >
                                    ...
                                    <div className="p-3 sm:p-6">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
                                            ...
                                            <div className="text-left sm:text-right">
                                                ...
                                                <p className="text-sm sm:text-lg font-bold text-accent-700">
                                        ...
                                        <div className="relative pt-4 pb-2">
                                            <div className="flex items-start justify-between relative">
                                                {/* Progress Line - centered on circles */}
                                                <div className="absolute left-0 right-0 h-0.5 bg-gray-200" style={{ zIndex: 0, top: '16px' }}>
                                                    <div
                                                        className="h-full bg-accent-700 transition-all duration-500"
                                                        style={{
                                                ...
                                                {progressSteps.map((step, index) => {
                                                    ...
                                                    return (
                                                        <div key={step.key} className="flex flex-col items-center relative w-0" style={{ zIndex: 1, flex: '1 1 0%' }}>
                                                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition-all ${step.completed
                                                            ...
                                                            <p className={`text-[9px] sm:text-xs font-medium text-center w-full mt-1 sm:mt-2 leading-tight ${step.completed || step.active ? 'text-gray-900' : 'text-gray-500'
                                                                }`}>
                                                                {step.label}
                                                            </p>
                                                        </div>
                                                                    ...
                                                                    <div className="flex items-center justify-between mb-1">
                                                                        <div className="flex items-center gap-1.5">
                                                                            ...
                                                                            {attempt.status === 'approved' && (
                                                                                <span className="inline-flex items-center gap-0.5 text-[9px] sm:text-[10px] bg-green-600 text-white px-1 sm:px-1.5 py-0.5 rounded-full font-medium">
                                                                                    <CheckCircle className="w-2 h-2 sm:w-2.5 sm:h-2.5" />
                                                                                    Approved
                                                                                </span>
                                                                            )}
                                                                            {attempt.status === 'declined' && (
                                                                                <span className="inline-flex items-center gap-0.5 text-[9px] sm:text-[10px] bg-red-600 text-white px-1 sm:px-1.5 py-0.5 rounded-full font-medium">
                                                                                    <XCircle className="w-2 h-2 sm:w-2.5 sm:h-2.5" />
                                                                                    Declined
                                                                                </span>
                                                                            )}
                                                                            {attempt.status === 'pending' && (
                                                                                <span className="inline-flex items-center gap-0.5 text-[9px] sm:text-[10px] bg-blue-600 text-white px-1 sm:px-1.5 py-0.5 rounded-full font-medium">
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className="text-accent-700 hover:text-accent-900 text-[10px] underline font-medium"
                                                                        >
                                                                            View
                                                                        </button>
                                                                    </div>
                                                                    <div className="text-[11px] text-gray-500 flex flex-col gap-0.5">
                                                                        <span>{new Date(attempt.uploadedAt).toLocaleString('en-PH')}</span>
                                                                        {attempt.declinedAt && (
                                                                            <span className="text-red-600">Declined {new Date(attempt.declinedAt).toLocaleString('en-PH')}</span>
                                                                        )}
                                                                        {attempt.approvedAt && (
                                                                            <span className="text-green-600">Approved {new Date(attempt.approvedAt).toLocaleString('en-PH')}</span>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ...
                                                        {previewImage && previewOrderId === order.id ? (
                                                            <div className="space-y-3">
                                                                <div className="relative border border-slate-200 rounded-lg p-3 bg-slate-50/30">
                                                                    <div className="flex items-center gap-1.5 mb-2">
                                                                        <Eye className="w-3.5 h-3.5 text-accent-700" />
                                                                        <span className="text-xs font-semibold text-accent-900">Preview</span>
                                                                    </div>
                                                            ...
                                                            <>
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="flex items-center gap-2 px-4 py-2 border-2 border-dashed border-slate-300 text-accent-800 rounded-lg hover:border-slate-500 hover:bg-slate-50 transition text-sm font-medium"
                                                                >
                                        ...
                                        {order.paymentStatus === 'cancelled' && order.cancellationReason && (
                                            <div className="mt-4 p-4 bg-red-50 border-2 border-red-300 rounded-lg">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex-1">
                                                        <h4 className="font-semibold text-red-900 mb-1" style={{ fontSize: '13px' }}>Order Cancelled</h4>
                                        ...
                                        <div className="mt-4 flex flex-wrap gap-3">
                                            ...
                                            <button
                                                ...
                                                className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition text-[12px] font-medium text-gray-700"
                                            >
                                                {isExpanded ? (
                                                    <>
                                            ...
                                            {order.paymentStatus === 'pending' && (
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold text-[12px]"
                                                >
                                            ...
                                            <div className="border-t border-gray-200 bg-gray-50 p-6">
                                                ...
                                                {order.paymentProofUrl && (
                                                    <div className="bg-white p-4 rounded-lg">
                                                        <h4 className="font-semibold text-gray-900 mb-3 text-[12px]">Proof of Payment</h4>
                                                        <button
                                                            onClick={() => {
                                                                setProofModal({ isOpen: true, imageUrl: order.paymentProofUrl });
                                                                setZoomLevel(1);
                                                            }}
                                                            className="flex items-center gap-2 text-accent-700 hover:text-accent-800 font-medium text-[12px]"
                                                        >
