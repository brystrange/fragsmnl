...
const StoreView = ({ collections, products, user, reserveProduct, logout, reservations, isAdmin, notifications, markNotificationRead, markAllNotificationsRead, dataLoading }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Loading Overlay */}
      {dataLoading?.collections || dataLoading?.products ? (
        ...
        <>
            ...
            {filteredProducts.length === 0 ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-5">
                {filteredProducts.map(product => {
                  ...
                  return (
                    <div key={product.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition group text-[12px] sm:text-sm">
                      ...
                      >
                        {product.imageUrl ? (
                          <img
                            ...
                            alt={product.name}
                            className="w-full h-36 sm:h-56 object-cover group-hover:scale-105 transition duration-300"
                          />
                        ) : (
                          <div className="w-full h-36 sm:h-56 flex items-center justify-center">
                            <ShoppingBag className="w-10 h-10 sm:w-16 sm:h-16 text-gray-300" />
                          </div>
                        )}
                      ...
                      <div className="p-2.5 sm:p-4">
                        ...
                        >
                          {collection && (
                            <p className="text-xs text-accent-700 font-medium mb-1">{collection.name}</p>
                          )}
                          <h3 className="font-semibold text-gray-900 mb-1 line-clamp-2 text-[11px] sm:text-sm min-h-[2rem] sm:min-h-[2.5rem]">
                            {product.name}
                          </h3>

                          {/* Price and Stock */}
                          <div className="flex items-center justify-between mb-3">
                            <span className="text-sm sm:text-lg font-bold text-gray-900">₱{product.price.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        ...
                        <button
                          ...
                          className={`w-full py-2 sm:py-2.5 rounded-lg font-semibold transition text-xs sm:text-sm ${product.availableStock > 0
                            ? isLoading
          ...
          {selectedProduct && (() => {
            ...
            return (
              ...
              >
                <div
                  className="bg-white rounded-lg shadow-2xl max-w-3xl w-full max-h-[90vh] sm:max-h-[85vh] overflow-y-auto"
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Close Button */}
                  <button
                    ...
                    <div className="flex flex-col">
                      ...
                      {collection && (
                        <div className="inline-flex items-center gap-1.5 self-start mb-2">
                          <Package className="w-3.5 h-3.5 text-accent-700" />
                          <span className="text-xs text-accent-700 font-semibold">{collection.name}</span>
                        </div>
                      )}
                      ...
                      <div className="mb-4 flex items-center gap-2">
                        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md ${currentProduct.availableStock > 0
                          ? 'bg-green-50 border border-green-200'
                          : 'bg-red-50 border border-red-200'
                          }`}>
                          <div className={`w-1.5 h-1.5 rounded-full ${currentProduct.availableStock > 0 ? 'bg-green-500' : 'bg-red-500'
                            }`}></div>
                          <span className={`text-xs font-semibold ${currentProduct.availableStock > 0 ? 'text-green-700' : 'text-red-700'
                            }`}>
                            {currentProduct.availableStock > 0
                      ...
                      <div className="mt-auto space-y-2">
                        <button
                          ...
                          className={`w-full py-2.5 rounded-md font-semibold text-sm transition ${currentProduct.availableStock > 0
                            ? isLoading
                              ? 'bg-slate-400 text-white cursor-wait'
                              : 'btn-accent'
                            : 'bg-gray-200 text-gray-500 cursor-not-allowed'
