...
// Wrapper components
const AdminPanelWrapper = ({ collections, products, addCollection, addProduct, updateProduct, deleteProducts, deleteCollection, logout, reservations, showToast, updateCollection, orders, paymentSettings, updatePaymentSettings, verifyOrderPayment }) => {
  return (
    <AdminPanel
...
const App = () => {
  ...
  const reserveProduct = async (productId, quantity = 1) => {
    ...
    const existingReservation = reservations.find(
      r => r.userId === user.id && r.productId === productId && r.status === 'active'
    );

    if (existingReservation) {
      showToast('You already have this product reserved', 'warning');
      return;
    }
