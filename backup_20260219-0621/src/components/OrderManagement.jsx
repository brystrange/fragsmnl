...
import { Search, Download, Eye, FileText, CheckCircle, XCircle, Clock, AlertCircle, MapPin, User, Phone, Mail, Image, Check, DollarSign, Package, ZoomIn, ZoomOut, Truck, Edit2, Save, X as XIcon } from 'lucide-react';
import { jsPDF } from 'jspdf';
import { useModalScrollLock } from '../hooks/useModalScrollLock';

const OrderManagement = ({ orders, showToast, verifyOrderPayment, declineOrderPayment, updateOrderTracking }) => {
  ...
  const downloadInvoice = (order) => {
    ...
    if (order.trackingNumber || order.courierName) {
      doc.setFont('helvetica', 'bold');
      doc.text('TRACKING INFORMATION', 20, 100);
      doc.setFont('helvetica', 'normal');
      if (order.courierName) doc.text(`Courier: ${order.courierName}`, 20, 108);
      if (order.trackingNumber) doc.text(`Tracking #: ${order.trackingNumber}`, 20, 114);
      itemsStartY = 124;
    }

    // Items Header
    doc.setFillColor(128, 0, 128);
    doc.rect(20, itemsStartY, 170, 8, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    doc.text('Item', 22, itemsStartY + 5);
    doc.text('Qty', 110, itemsStartY + 5);
    doc.text('Unit Price', 130, itemsStartY + 5);
    doc.text('Total', 165, itemsStartY + 5);
              ...
              <div className="border rounded-lg divide-y">
                {selectedOrder.items.map((item, i) => (
                  <div key={i} className="p-3 flex justify-between">
                    ...
                    <div className="font-semibold text-slate-600">₱{item.totalPrice.toFixed(2)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-3">
              {selectedOrder.paymentStatus === 'payment_submitted' && verifyOrderPayment && (
                <button
                  ...
                  disabled={verifying}
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-1.5 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50"
                >
              ...
              {selectedOrder.paymentStatus === 'payment_submitted' && declineOrderPayment && (
                <button
                  ...
                  disabled={declining}
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-1.5 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50"
                >
                  <XCircle className="w-4 h-4" />
                  {declining ? 'Declining...' : 'Decline Proof'}
                </button>
              )}
              <button onClick={() => downloadInvoice(selectedOrder)} className="flex-1 flex items-center justify-center gap-2 px-3 py-1.5 text-sm bg-slate-600 text-white rounded-lg hover:bg-slate-700">
                <Download className="w-4 h-4" />
                Download Invoice
              </button>
              <button onClick={() => setSelectedOrder(null)} className="px-3 py-1.5 text-sm border rounded-lg hover:bg-gray-50">Close</button>
            </div>
          </div>
        </div>,
        document.body
