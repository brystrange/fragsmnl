...
const Checkout = ({ reservations, products, user, logout, isAdmin, collections, createOrder, notifications, markNotificationRead, markAllNotificationsRead }) => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        >
          ...
          {step === 1 ? 'Back to Reservations' : 'Back to Item Selection'}
        </button>

        {/* Step Indicator */}
        <div className="flex items-center justify-center mb-8">
          <div className="flex items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold ${step >= 1 ? 'bg-slate-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
              1
            </div>
            <span className={`ml-2 mr-4 font-medium ${step >= 1 ? 'text-slate-600' : 'text-gray-400'}`}>Select Items</span>
          </div>
          <div className="w-12 h-0.5 bg-gray-300"></div>
          <div className="flex items-center ml-4">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold ${step >= 2 ? 'bg-slate-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
              2
            </div>
            <span className={`ml-2 font-medium ${step >= 2 ? 'text-slate-600' : 'text-gray-400'}`}>Shipping Details</span>
          </div>
        </div>
                  ...
                  <div className="space-y-4">
                    {reservations.map(reservation => {
                      ...
                      return (
                        <div
                          key={reservation.id}
                          className={`border rounded-lg p-4 transition ${isSelected ? 'border-slate-500 bg-slate-50' : 'border-gray-200'
                            }`}
                        >
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <div>
                        <span className="font-medium text-gray-900">Use a different shipping address</span>
                        <p className="text-sm text-gray-600">Enter a different address for delivery</p>
                      </div>
                    </label>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent ${useDifferentAddress ? 'bg-white' : 'bg-gray-100'
                          }`}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        disabled={!useDifferentAddress}
                        rows={3}
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent resize-none ${useDifferentAddress ? 'bg-white' : 'bg-gray-100'
                          }`}
                      ...
                      {getSelectedReservations().map(reservation => {
                        ...
                        return (
                          <div key={reservation.id} className="flex justify-between text-sm">
                            <span className="text-gray-600 truncate mr-2">{product.name} × {reservation.quantity || 1}</span>
                            <span className="font-medium">₱{(product.price * (reservation.quantity || 1)).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div className="bg-white rounded-lg shadow p-6 sticky top-6">
                ...
                {step === 1 ? (
                  <button
                    ...
                    className={`w-full py-3 rounded-lg font-semibold transition flex items-center justify-center gap-2 ${selectedCount > 0
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-lg font-semibold transition ${selectedCount > 0 && !isSubmitting
