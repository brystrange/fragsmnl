...
const StoreViewWrapper = ({ collections, products, user, reserveProduct, logout, reservations, isAdmin, notifications, markNotificationRead, markAllNotificationsRead }) => {
  return (
    <StoreView
      collections={collections}
      products={products}
...
const MyReservationsWrapper = ({ reservations, products, cancelReservation, logout, user, isAdmin, collections, notifications, markNotificationRead, markAllNotificationsRead }) => {
  return (
    <MyReservations
      reservations={reservations}
      products={products}
...
const App = () => {
  ...
  // Auto-cancel orders without payment proof after 48 hours + send warning at 24hrs
  useEffect(() => {
    const checkExpiredOrders = async () => {
      ...
      for (const order of orders) {
        ...
        if (order.paymentStatus === 'pending' && !order.paymentProofUrl) {
          ...
          // If order is older than 24 hours but less than 48, send warning (once)
          else if (orderCreatedAt < twentyFourHoursAgo && !order.cancelWarningNotified) {
            await addNotification(order.userId, {
              type: 'order_cancel_warning',
              title: 'Order About to Be Cancelled',
              message: `Your order #${order.orderNumber} will be auto-cancelled in less than 24 hours if no payment proof is uploaded.`,
              orderId: order.id,
