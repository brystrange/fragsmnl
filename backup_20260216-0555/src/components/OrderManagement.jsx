            ...
            {selectedOrder.paymentAttempts && selectedOrder.paymentAttempts.length > 0 && (
              <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                <h3 className="font-semibold mb-3 flex items-center gap-2 text-gray-800">
                  ...
                  Payment Proof History ({selectedOrder.paymentAttempts.length} attempt{selectedOrder.paymentAttempts.length > 1 ? 's' : ''})
                </h3>
                <div className="space-y-3">
                  {selectedOrder.paymentAttempts.map((attempt, index) => (
                    <div key={index} className={`p-3 rounded-lg border-2 ${
                      attempt.status === 'approved' ? 'bg-green-50 border-green-300' :
                      attempt.status === 'declined' ? 'bg-red-50 border-red-300' :
